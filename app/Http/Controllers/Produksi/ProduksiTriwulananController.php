...
use App\Models\Produksi\ProduksiTriwulanan; // Ganti Model
...
use App\Exports\ProduksiTriwulananExport;   // Ganti Export
use App\Imports\ProduksiTriwulananImport;   // Ganti Import
...
class ProduksiTriwulananController extends Controller
{
    ...
    public function index(Request $request, $jenisKegiatan)
    {
        ...
        $currentModul = 'produksi_triwulanan'; // Ganti Modul
        ...
        $availableTahunQuery = ProduksiTriwulanan::query() // Ganti Model
        ...
        $availableTahun = $availableTahunQuery
            ->select(DB::raw('YEAR(produksi_triwulanan.created_at) as tahun'))
            ->distinct()
            ->whereNotNull('produksi_triwulanan.created_at')
            ->orderBy('tahun', 'desc')
            ->pluck('tahun')
            ->toArray();
        ...
        // 3. Kueri Utama
        $query = ProduksiTriwulanan::query() // Ganti Model
            ->leftJoin('master_kegiatan', 'produksi_triwulanan.master_kegiatan_id', '=', 'master_kegiatan.id_master_kegiatan')
            ->where(function($q) use ($currentModul) {
                $q->where('master_kegiatan.modul', $currentModul)
                ->orWhereNull('produksi_triwulanan.master_kegiatan_id'); 
            ...
            ->whereYear('produksi_triwulanan.created_at', $selectedTahun);

        // Filter Kegiatan Spesifik (Tab) 
        $selectedKegiatan = $request->input('kegiatan', '');
        if ($selectedKegiatan !== '') {
            if (is_numeric($selectedKegiatan)) {
                $query->where('produksi_triwulanan.master_kegiatan_id', $selectedKegiatan);
            } else {
                $query->whereNull('produksi_triwulanan.master_kegiatan_id')
                    ->where('produksi_triwulanan.nama_kegiatan', $selectedKegiatan);
            }
        }
        ...
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $q->where('produksi_triwulanan.BS_Responden', 'like', "%{$search}%")
                ->orWhere('produksi_triwulanan.pencacah', 'like', "%{$search}%")
                ->orWhere('produksi_triwulanan.pengawas', 'like', "%{$search}%")
                ->orWhere('master_kegiatan.nama_kegiatan', 'like', "%{$search}%") 
                ->orWhere('produksi_triwulanan.nama_kegiatan', 'like', "%{$search}%");
            });
        }
        ...
        if ($perPageInput == 'all') {
            ...
            $total = $countQuery->count('produksi_triwulanan.id_produksi_triwulanan'); // Ganti ID
            $perPage = $total > 0 ? $total : 20; 
        }

        // 5. Ambil Data
        $listData = $query
            ->select('produksi_triwulanan.*') 
            ->with('masterKegiatan') 
            ->latest('produksi_triwulanan.id_produksi_triwulanan') // Ganti ID
            ->paginate($perPage) 
            ->withQueryString(); 

        // 6. Logika Hitung Tab (Dashboard)
        $kegiatanCountsQuery = ProduksiTriwulanan::query() // Ganti Model
            ->leftJoin('master_kegiatan', 'produksi_triwulanan.master_kegiatan_id', '=', 'master_kegiatan.id_master_kegiatan')
            ->where(function($q) use ($currentModul) {
                $q->where('master_kegiatan.modul', $currentModul)
                ->orWhereNull('produksi_triwulanan.master_kegiatan_id'); 
        ...
        $kegiatanCounts = $kegiatanCountsQuery
            ->select(
                DB::raw('COALESCE(master_kegiatan.id_master_kegiatan, produksi_triwulanan.nama_kegiatan) as filter_value'),
                DB::raw('COALESCE(master_kegiatan.nama_kegiatan, produksi_triwulanan.nama_kegiatan) as display_name'),
                DB::raw('count(produksi_triwulanan.id_produksi_triwulanan) as total') // Ganti ID
            )
        ...
        // 7. Kirim ke View
        return view('timProduksi.ProduksiTriwulanan', compact( // GANTI VIEW
            'listData', 
            'kegiatanCounts', 
            'jenisKegiatan',      
            'masterKegiatanList', 
    ...
    public function store(Request $request)
    {
        ...
        ProduksiTriwulanan::create($validatedData); // Ganti Model
    ...
    {
        $produksi_triwulanan = ProduksiTriwulanan::findOrFail($id); // Ganti Model
    ...
    public function update(Request $request, $id) 
    {
        ...
        if ($validator->fails()) {
            ...
            return back()->withErrors($validator, 'editForm') 
                ...
                ->with('edit_id', $produksi_triwulanan->id_produksi_triwulanan); // Ganti ID
        }
    ...
    public function bulkDelete(Request $request)
    {
        $request->validate([
            'ids'   => 'required|array',
            'ids.*' => 'exists:produksi_triwulanan,id_produksi_triwulanan' // GANTI TABEL/ID
        ]);
        ProduksiTriwulanan::whereIn('id_produksi_triwulanan', $request->ids)->delete(); // GANTI MODEL/ID
    ...
    public function searchKegiatan(Request $request, $jenisKegiatan = null)
    {
        ...
        $kegiatanQuery->where('modul', 'produksi_triwulanan'); // Ganti Modul
    ...
    public function export(Request $request, $jenisKegiatan)
    {
        ...
        $exportClass = new ProduksiTriwulananExport( // Ganti Export
            $dataRange,
            $dataFormat,
            $jenisKegiatan,   
            $kegiatan,        
        ...
        $fileName = 'ProduksiTriwulanan_' . str_replace(' ', '_', strtoupper($jenisKegiatan)) . '_' . $selectedTahun . '_' . now()->format('YmdHis'); // Ganti Nama File
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $import = new ProduksiTriwulananImport(); // Ganti Import
    ...
    public function downloadTemplate()
    {
        try {
            ...
            $instructions = [
                ...
                '2. "nama_kegiatan" WAJIB terdaftar di Master Kegiatan (modul produksi_triwulanan).',
            ...
            $fileName = 'Template_Import_Produksi_Triwulanan.xlsx'; // Ganti Nama File
