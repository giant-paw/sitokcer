...
class SosialSemesteranExport implements FromCollection, WithHeadings
{
    protected $dataRange;
    ...
    public function collection()
    {
        ...
        // Tentukan prefix berdasarkan jenis kegiatan
        $jenisKegiatanLower = strtolower($this->jenisKegiatan);
        $prefixKegiatan = ($jenisKegiatanLower == 'sakernas') ? 'Sakernas' : 'Susenas';

        // Filter berdasarkan jenis kegiatan
        $query->where('nama_kegiatan', 'LIKE', $prefixKegiatan . '%');

        // Filter berdasarkan tahun
        $query->whereYear('created_at', $this->tahun);

        // Filter berdasarkan kegiatan spesifik (misal: Sakernas Semester 1)
        ...
        $query->latest('id_sosial_semesteran');
    ...
    {
        $templatePath = storage_path('templates/sosial_semesteran_template.docx');
        ...
        $jenisKegiatanTitle = ucfirst($this->jenisKegiatan);
        $judulLaporan = 'Laporan Sosial Semesteran (' . $jenisKegiatanTitle . ') Tahun ' . $this->tahun;
        if (!empty($this->kegiatan)) {
        ...
        $placeholderToClone = 'id_sosial_semesteran';
        ...
        if ($dataCount > 0) {
            ...
            foreach ($data as $index => $row) {
                ...
                $templateProcessor->setValue('id_sosial_semesteran#' . $i, $row->id_sosial_semesteran ?? '');
            ...
            try {
                ...
                $templateProcessor->setValue('id_sosial_semesteran#1', 'Tidak ada data');
