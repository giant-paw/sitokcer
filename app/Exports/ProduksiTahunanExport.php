...
{{-- Judul Disesuaikan --}}
@section('title', strtoupper(str_replace('-', ' ', $jenisKegiatan)) . ' - Produksi Tahunan - Sitokcer')
...
@section('content')
    ...
    {{-- Alert Import Errors --}}
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                <h2 class="page-title">List Target Kegiatan Tahunan: {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }}</h2>
        ...
        <div class="data-card">
            ...
            <div class="toolbar">
                ...
                <div class="toolbar-right">
                    ...
                    <form action="{{ route('tim-produksi.tahunan.index', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" class="search-form">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_produksi }}"></td>
                                ...
                                {{-- Penanganan Tanggal Kotor (VARCHAR) --}}
                                @php
                                    try {
                                        $target_penyelesaian = \Carbon\Carbon::parse($item->target_penyelesaian)->format('d/m/Y');
                                    } catch (\Exception $e) {
                                        $target_penyelesaian = '-';
                                    }
                                    try {
                                        $tanggal_pengumpulan = \Carbon\Carbon::parse($item->tanggal_pengumpulan)->format('d/m/Y');
                                    } catch (\Exception $e) {
                                        $tanggal_pengumpulan = '-';
                                    }
                                @endphp
                                <td class="text-secondary">{{ $target_penyelesaian }}</td>
                                <td>
                                ...
                                <td class="text-secondary">{{ $tanggal_pengumpulan }}</td>
                                <td class="td-action">
                                    <div class="action-buttons">
                                        <button class="btn-icon btn-icon-edit" title="Edit" data-bs-toggle="modal" data-bs-target="#editDataModal" onclick="editData({{ $item->id_produksi }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg> </button>
                                        <button class="btn-icon btn-icon-delete" title="Hapus" data-bs-toggle="modal" data-bs-target="#deleteDataModal" onclick="deleteData({{ $item->id_produksi }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> </button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    @if($search || $selectedKegiatan) <a href="{{ route('tim-produksi.tahunan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <form action="{{ route('tim-produksi.tahunan.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info d-flex align-items-start p-3" role="alert">
                            ...
                            <div>
                                <small>
                                    <strong>PENTING:</strong> Pastikan data `nama_kegiatan` di file Excel Anda **sudah valid** dan terdaftar di Master Kegiatan untuk modul **Produksi Tahunan**.
                                </small>
                            </div>
                        </div>
                        <div class="mb-3">
                            <a href="{{ route('tim-produksi.tahunan.downloadTemplate') }}" class="btn-action btn-secondary w-100">
    ...
    <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('tim-produksi.tahunan.export', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" id="exportForm">
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data Tahunan {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }}</h5> <p class="modal-subtitle">Pilih opsi ekspor</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
    ...
    <div class="modal fade" id="editDataModal" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-lg modal-dialog-centered"> 
            <form id="editForm" method="POST"> 
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Edit Data Tahunan</h5> <p class="modal-subtitle">Perbarui informasi data</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
...
{{-- 
====================================================================
== SEMUA 'caturwulan' TELAH DIUBAH MENJADI 'tahunan' ==
...
<script>
    ...
    // --- [PERBAIKAN URL BASIS] ---
    const tahunanRootUrl = `{{ url('tim-produksi/tahunan') }}`;
    const tahunanPageUrl = `{{ route('tim-produksi.tahunan.index', ['jenisKegiatan' => $jenisKegiatan]) }}`;
    ...
    function editData(id) {
        ...
        editForm.action = `${tahunanRootUrl}/${id}`;
    ...
    function deleteData(id) {
        ...
        deleteForm.action = `${tahunanRootUrl}/${id}`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(Route::has('tim-produksi.tahunan.searchKegiatan'))
            const kegiatanSearchUrl = `{{ route("tim-produksi.tahunan.searchKegiatan", ['jenisKegiatan' => $jenisKegiatan]) }}?`;
        ...
        @else 
            console.warn('Rute tim-produksi.tahunan.searchKegiatan tidak ditemukan.'); 
        @endif
        ...
        @else 
            console.warn('Rute tim-produksi.tahunan.searchPetugas tidak ditemukan.'); 
        @endif
        ...
        function hfc() {
            const cu = new URL(tahunanPageUrl);
        ...
        @if (session('error_modal') == 'editDataModal' && $errors->getBag('editForm')->any() && session('edit_id')) 
            ...
            if (eid_fb) { 
                ...
                if(emef) { 
                    ...
                    edf.action = `${tahunanRootUrl}/${eid_fb}`; 
