...
class ProduksiTahunanImport implements ToCollection, WithHeadingRow, SkipsOnError
{
    ...
    {
        $this->masterKegiatanMap = MasterKegiatan::where('modul', 'produksi_tahunan')
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            try {
                ...
                ProduksiTahunan::create($validation['data']);
                $this->successCount++;
    ...
    protected function validateRow($row, $rowNumber)
    {
        ...
        if (!isset($this->masterKegiatanMap[$namaKegiatan])) {
            return ['valid' => false, 'message' => "Nama Kegiatan '{$namaKegiatan}' tidak terdaftar di master (modul produksi_tahunan)."];
        }
    ...
    protected function parseDate($date, $isNullable = false)
    {
        ...
        $formats = ['Y-m-d', 'd/m/Y', 'd-m-Y', 'Y/m/d', 'Y-m-d H:i:s', 'Y-n-j G:i:s']; // Menambahkan format dari SQL dump
