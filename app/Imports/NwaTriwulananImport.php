...
class NwaTriwulananImport implements ToCollection, WithHeadingRow, SkipsOnError
{
    use SkipsErrors;
    ...
    {
        $this->masterKegiatanMap = MasterKegiatan::where('modul', 'nwa_triwulanan') 
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            try {
                ...
                NwaTriwulanan::create($validation['data']); 
                $this->successCount++;
    ...
    protected function validateRow($row, $rowNumber)
    {
        ...
        if (!isset($this->masterKegiatanMap[$namaKegiatan])) {
            return ['valid' => false, 'message' => "Nama Kegiatan '{$namaKegiatan}' tidak terdaftar di master (modul nwa_triwulanan)."]; 
        }
