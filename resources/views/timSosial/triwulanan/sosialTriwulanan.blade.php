...
@section('header-title', 'List Target Kegiatan Seruti')
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header bg-light">
                {{-- Judul Disesuaikan --}}
                <h4 class="card-title mb-0">LIST TARGET KEGIATAN SERUTI</h4>
            </div>
            <div class="card-body">
                ...
                {{-- Tab Kegiatan (misal: Seruti-TW1, TW2, ...) --}}
                <ul class="nav nav-pills mb-3 d-flex flex-wrap gap-2" >
                    <li class="nav-item">
                        {{-- Link ke index dengan jenisKegiatan 'seruti' --}}
                        <a class="nav-link {{ empty($selectedKegiatan) ? 'active' : '' }}"
                           href="{{ route('sosial.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}">
                           Semua TW
                        </a>
                    </li>
                    @foreach($kegiatanCounts ?? [] as $kegiatan)
                        <li class="nav-item">
                            <a class="nav-link {{ ($selectedKegiatan ?? '') == $kegiatan->nama_kegiatan ? 'active' : '' }}"
                               href="{{ route('sosial.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'kegiatan' => $kegiatan->nama_kegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}">
                ...
                {{-- Form Pencarian --}}
                <form action="{{ route('sosial.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" class="mb-4">
                        ...
                        <tbody>
                            @forelse ($listData ?? [] as $item)
                                <tr>
                                    ...
                                    <td class="text-center"><input type="checkbox" class="form-check-input row-checkbox" value="{{ $item->id_sosial_triwulanan }}"></td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm">
                                            {{-- Primary key disesuaikan --}}
                                            <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editDataModal" onclick="editData({{ $item->id_sosial_triwulanan }})"><i class="bi bi-pencil-square"></i></button>
                                            <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteDataModal" onclick="deleteData({{ $item->id_sosial_triwulanan }})"><i class="bi bi-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center">Tidak ada data Seruti yang ditemukan untuk tahun {{ $selectedTahun }}.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="tambahDataModalLabel">Tambah Data Seruti Baru</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    {{-- Nama Kegiatan (Autocomplete difilter hanya Seruti) --}}
                    <div class="mb-3 autocomplete-container">
                        <label for="nama_kegiatan" class="form-label">Nama Kegiatan <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('nama_kegiatan') is-invalid @enderror" id="nama_kegiatan" name="nama_kegiatan" value="{{ old('nama_kegiatan') }}" placeholder="Ketik Seruti-TW..." required autocomplete="off">
                        ...
                        <small class="form-text text-muted">Contoh: Seruti-TW1, Seruti-TW2, dst.</small>
                    </div>
    ...
    <div class="modal fade" id="editDataModal" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form id="editForm" method="POST"> {{-- Action diatur JS --}}
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editDataModalLabel">Edit Data Seruti</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
...
<script>
    ...
    function editData(id) {
        ...
        fetch(`${sosialTriwulananBaseUrl}/${id}/edit`)
            ...
            .catch(error => { console.error("Error loading edit data:", error); alert('Tidak dapat memuat data Seruti. Error: ' + error.message); });
    }
    ...
    function deleteData(id) {
        ...
        document.getElementById('deleteModalBody').innerText = 'Apakah Anda yakin ingin menghapus data Seruti ini?'; // Teks disesuaikan
