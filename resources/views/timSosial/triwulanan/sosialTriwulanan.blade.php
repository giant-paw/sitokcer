...
{{-- Judul Disesuaikan --}}
@section('title', 'Sosial Triwulanan - ' . Str::studly($jenisKegiatan))
@section('header-title', 'List Target Kegiatan Triwulanan Tim Sosial (' . Str::studly($jenisKegiatan) . ')')
...
@push('styles')
    <style>
        ...
        /* Lebar kolom tabel - DIPERBAIKI */
        .data-table { table-layout: fixed; width: 100%; border-collapse: separate; border-spacing: 0; }
        .th-checkbox { width: 5%; text-align: center; }
        
        .data-table th:nth-child(2) { width: 20%; } /* Nama Kegiatan */
        .data-table th:nth-child(3) { width: 13%; } /* BS/Responden (Dikurangi sedikit) */
        ...
        .data-table th:nth-child(7) { width: 12%; } /* Progress (DIPERLEBAR dari 8% ke 12%) */
        .data-table th:nth-child(8) { width: 10%; } /* Tgl Kumpul */
        .th-action { width: 6%; text-align: center; }  /* Aksi */
        ...
        .badge {
            ...
            line-height: 1;
            text-align: center;
            white-space: nowrap; /* PENTING: Agar 'BELUM SELESAI' tidak turun baris */
        ...
        .badge-warning {
            color: #000; /* Hitam (agar kontras terbaca di kuning) */
        ...
        /* Tampilkan invalid feedback di modal */
...
@section('content')
    ...
    {{-- Alert Import Errors --}}
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                 <h2 class="page-title">List Target Kegiatan Triwulanan ({{ Str::studly($jenisKegiatan) }})</h2>
                 <p class="page-subtitle">Kelola data target kegiatan triwulanan tim Sosial</p>
            </div>
        </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_sosial_triwulanan }}"></td>
                                ...
                                <td>
                                    @php
                                        $badgeClass = 'badge-warning'; // Default Belum Selesai / Proses
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    <p class="empty-text">Tidak ada data triwulanan ditemukan untuk tahun {{ $selectedTahun }}.</p>
                                     @if($search || $selectedKegiatan) <a href="{{ route('sosial.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modern-modal">
                <form action="{{ route('sosial.triwulanan.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="jenisKegiatan" value="{{ $jenisKegiatan }}">
                    <div class="modal-header">
                         <div class="modal-header-content">
                             <h5 class="modal-title" id="importModalLabel">Import Data dari Excel</h5>
                             <p class="modal-subtitle">Upload file sesuai template untuk {{ Str::studly($jenisKegiatan) }}</p>
                         </div>
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info d-flex align-items-start p-3" role="alert">
                            ...
                            <div>
                                <small>
                                    <strong>PENTING:</strong> Pastikan data `nama_kegiatan` di file Excel Anda **sudah valid** dan terdaftar di Master Kegiatan (contoh: 'Seruti-TW1', 'Seruti-TW2', dll).
                                </small>
                            </div>
                        </div>
                        <div class="mb-3">
                            <a href="{{ route('sosial.triwulanan.downloadTemplate', ['jenisKegiatan' => $jenisKegiatan]) }}" class="btn-action btn-secondary w-100">
    ...
    <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
             <form action="{{ route('sosial.triwulanan.export', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" id="exportForm">
                {{-- Parameter jenisKegiatan sudah ada di URL action --}}
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data {{ Str::studly($jenisKegiatan) }}</h5> <p class="modal-subtitle">Pilih opsi ekspor</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
    ...
    <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-lg modal-dialog-centered"> 
            <form action="{{ route('sosial.triwulanan.store') }}" method="POST" id="tambahForm">
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Tambah Data Triwulanan Baru</h5> <p class="modal-subtitle">Isi form di bawah</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="form-group autocomplete-container">
                            ...
                            <input type="text" class="form-input @error('nama_kegiatan') is-invalid @enderror"
                                ...
                                placeholder="Ketik nama kegiatan (contoh: {{ Str::studly($jenisKegiatan) }}-TW1)..." required autocomplete="off">
    ...
    <div class="modal fade" id="editDataModal" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-lg modal-dialog-centered"> 
            <form id="editForm" method="POST"> {{-- Action di-set oleh JS --}}
                @csrf 
                @method('PUT')
                ...
                {{-- jenisKegiatan tidak diperlukan di form edit, karena data sudah ada --}}
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Edit Data Triwulanan</h5> <p class="modal-subtitle">Perbarui informasi data</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
    ...
    <div class="modal fade" id="deleteDataModal" tabindex="-1" aria-labelledby="deleteDataModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <form id="deleteForm" method="POST"> {{-- Action di-set oleh JS --}}
                @csrf
                ...
                <div class="modal-content modern-modal">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="delete-text" id="deleteModalBody">Hapus data Triwulanan ini?</p> 
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn-secondary" data-bs-dismiss="modal">Batal</button>
...
@push('scripts')
{{-- Skrip ini diadaptasi untuk menangani parameter jenisKegiatan --}}
<script>
    /**
     * [VERSI ADAPTASI] Autocomplete "Pintar"
     * Sekarang menyertakan 'jenisKegiatan' dalam pencarian.
     */
    function initAutocomplete(inputId, suggestionsId, searchUrl) {
        ...
        input.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                // searchUrl sudah berisi ?jenisKegiatan=...
    ...
    // --- URL Basis BARU ---
    // Rute {id} tidak memerlukan {jenisKegiatan} berdasarkan file route Anda
    const triwulananBaseUrl = `{{ route('sosial.triwulanan.index') }}`; // Akan menjadi /sosial/triwulanan
    ...
    function editData(id) {
        ...
        editForm.action = `${triwulananBaseUrl}/${id}`; // Action: /sosial/triwulanan/{id}
        ...
        fetch(`${triwulananBaseUrl}/${id}/edit`) // Action: /sosial/triwulanan/{id}/edit
            .then(response => response.ok ? response.json() : Promise.reject(response))
            .then(data => {
    ...
    function deleteData(id) {
        ...
        deleteForm.action = `${triwulananBaseUrl}/${id}`; // Action: /sosial/triwulanan/{id}
        ...
        document.getElementById('deleteModalBody').innerText = `Apakah Anda yakin ingin menghapus data Triwulanan ini?`;
        
        const confirmBtn = document.getElementById('confirmDeleteButton');
        const newConfirmBtn = confirmBtn.cloneNode(true); // Re-clone untuk hapus event listener lama
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- Init Autocomplete (Kirim jenisKegiatan) ---
        @if(Route::has('sosial.triwulanan.searchKegiatan'))
            const kegiatanSearchUrl = `{{ route("sosial.triwulanan.searchKegiatan") }}?jenisKegiatan=${currentJenisKegiatan}&`; 
        ...
        @if(Route::has('sosial.triwulanan.searchPetugas'))
            const petugasSearchUrl = '{{ route("sosial.triwulanan.searchPetugas") }}?'; // Petugas tidak perlu filter
        ...
        bdb?.addEventListener('click', () => {
            ...
            df.action = '{{ route("sosial.triwulanan.bulkDelete") }}'; 
            let mi = df.querySelector('input[name="_method"]'); 
            if (mi) mi.remove(); // Pastikan method-nya POST

            df.querySelectorAll('input[name="ids[]"]').forEach(i => i.remove()); 
            // Tambahkan input jenisKegiatan untuk bulk delete (jika diperlukan controller)
            // const jkInput = document.createElement('input'); 
            // jkInput.type = 'hidden'; jkInput.name = 'jenisKegiatan'; jkInput.value = currentJenisKegiatan;
            // df.appendChild(jkInput);
        ...
        function hfc() {
            // [PERBAIKAN] Gunakan URL basis yang benar DENGAN jenisKegiatan
            const cu = new URL('{{ route("sosial.triwulanan.index", ["jenisKegiatan" => $jenisKegiatan]) }}');
        ...
        @if (session('error_modal') == 'editDataModal' && $errors->getBag('editForm')->any() && session('edit_id')) 
            ...
            if (eid_fb) { 
                ...
                if(emef) { 
                    ...
                    edf.action = `${triwulananBaseUrl}/${eid_fb}`; 
                    editData(eid_fb); // Memuat data dan memanggil .show()
