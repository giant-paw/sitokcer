        ...
        <div class="data-card">
            ...
            <div class="toolbar">
                <div class="toolbar-left">
                    ...
                    {{-- <button type="button" class="btn-action btn-secondary">...</button> --}} {{-- Import dikomentari --}}
            ...
            <div class="px-4 pt-4">
                 <ul class="nav nav-tabs">
                    <li class="nav-item">
                        <a class="nav-link {{ empty($selectedKegiatan) ? 'active' : '' }}" href="{{ route('sosial.tahunan.index', ['tahun' => $selectedTahun ?? date('Y')]) }}"> Semua Kegiatan </a>
                    </li>
                    @foreach($kegiatanCounts ?? [] as $kegiatan)
                        <li class="nav-item">
                            <a class="nav-link {{ ($selectedKegiatan ?? '') == $kegiatan->nama_kegiatan ? 'active' : '' }}" href="{{ route('sosial.tahunan.index', ['kegiatan' => $kegiatan->nama_kegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData ?? [] as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_sosial }}"></td> {{-- ID Sosial --}}
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    @if($search || $selectedKegiatan) <a href="{{ route('sosial.tahunan.index', ['tahun' => $selectedTahun ?? date('Y')]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('sosial.tahunan.export') }}" method="GET" id="exportForm">
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data Sosial Tahunan</h5> <p class="modal-subtitle">Pilih opsi ekspor Anda</p> </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group"> <label for="exportDataRangeSos" class="form-label">Jangkauan Data</label> <select class="form-select" id="exportDataRangeSos" name="dataRange" required> <option value="all">Semua Data ({{ $listData->total() }})</option> <option value="current_page">Halaman Ini ({{ $listData->count() }})</option> </select> </div>
                        <div class="form-group"> <label for="exportDataFormatSos" class="form-label">Format Nilai Tanggal</label> <select class="form-select" id="exportDataFormatSos" name="dataFormat"> <option value="formatted_values" selected>Format Tampilan</option> <option value="raw_values">Nilai Asli Database</option> </select> <small class="form-text text-muted">Pilih "Raw Values" untuk olah data.</small> </div>
                        <div class="form-group"> <label for="exportExportFormatSos" class="form-label">Format File</label> <select class="form-select" id="exportExportFormatSos" name="exportFormat" required> <option value="excel">Excel (.xlsx)</option> <option value="csv">CSV (.csv)</option> </select> </div>
                    </div>
                    <div class="modal-footer">
