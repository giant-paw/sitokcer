{{-- resources/views/Rekapitulasi/pengawas/print.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Laporan Rekapitulasi Pengawas</title>
    <style>
        ...
        .detail-table th { background-color: #fff; }
...
<body onload="window.print()">
    <div class="container">
        <h2 class="text-center mb-3">Laporan Rekapitulasi Pengawas</h2>
        @if($q)
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="text-start" style="width: 45%;">Pengawas</th>
                    <th class="text-center" style="width: 15%;">Total Responden</th>
                    <th class="text-start" style="width: 35%;">Detail Pencacah</th>
                </tr>
            </thead>
            <tbody>
                @forelse($rekapPengawas as $index => $pengawas)
                    <tr class="main-row">
                        ...
                        <td>
                            @if(count($pengawas->pencacah_list) > 0)
                                <table class="detail-table">
                                    ...
                                    <tbody>
                                        @foreach($pengawas->pencacah_list as $pencacah)
                                            <tr>
                                                <td>{{ $pencacah->pencacah }}</td>
                                                <td class="text-center">{{ $pencacah->jumlah_responden }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            @else
                                <span style="font-size: 11px; color: #6c757d;">Tidak ada pencacah.</span>
                            @endif
                        </td>
                    </tr>
                @empty
