...
{{-- Judul Disesuaikan --}}
@section('title', strtoupper($jenisKegiatan) . ' - Distribusi Triwulanan - Sitokcer')
    ...
    <div class="container-fluid px-4 py-4">
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                ...
                <p class="page-subtitle">Kelola data target kegiatan triwulanan {{ $jenisKegiatan }} tim Distribusi</p>
            </div>
        </div>
        ...
        <div class="data-card">
            ...
            <div class="px-4 pt-4">
                 <ul class="nav nav-tabs">
                    <li class="nav-item">
                        <a class="nav-link {{ empty($selectedKegiatan) ? 'active' : '' }}" href="{{ route('tim-distribusi.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}"> Semua TW ({{ strtoupper($jenisKegiatan) }}) </a>
                    </li>
                    @foreach($kegiatanCounts ?? [] as $kegiatan)
                        <li class="nav-item">
                            <a class="nav-link {{ ($selectedKegiatan ?? '') == $kegiatan->nama_kegiatan ? 'active' : '' }}" href="{{ route('tim-distribusi.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'kegiatan' => $kegiatan->nama_kegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData ?? [] as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_distribusi_triwulanan }}"></td> {{-- ID Triwulanan --}}
                                ...
                                <td class="td-action">
                                    <div class="action-buttons">
                                        <button class="btn-icon btn-icon-edit" title="Edit" data-bs-toggle="modal" data-bs-target="#editDataModal" onclick="editData({{ $item->id_distribusi_triwulanan }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg> </button>
                                        <button class="btn-icon btn-icon-delete" title="Hapus" data-bs-toggle="modal" data-bs-target="#deleteDataModal" onclick="deleteData({{ $item->id_distribusi_triwulanan }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> </button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    @if($search || $selectedKegiatan) <a href="{{ route('tim-distribusi.triwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    {{-- ================================================= --}}
     ...
     <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('tim-distribusi.triwulanan.export', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" id="exportForm">
                 <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data Triwulanan {{ strtoupper($jenisKegiatan) }}</h5> <p class="modal-subtitle">Pilih opsi ekspor Anda</p> </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group"> <label for="exportDataRangeDistTri" class="form-label">Jangkauan Data</label> <select class="form-select" id="exportDataRangeDistTri" name="dataRange" required> <option value="all">Semua Data ({{ $listData->total() }})</option> <option value="current_page">Halaman Ini ({{ $listData->count() }})</option> </select> </div>
                        <div class="form-group"> <label for="exportDataFormatDistTri" class="form-label">Format Nilai Tanggal</label> <select class="form-select" id="exportDataFormatDistTri" name="dataFormat"> <option value="formatted_values" selected>Format Tampilan</option> <option value="raw_values">Nilai Asli Database</option> </select> <small class="form-text text-muted">Pilih "Raw Values" untuk olah data.</small> </div>
                        <div class="form-group"> <label for="exportExportFormatDistTri" class="form-label">Format File</label> <select class="form-select" id="exportExportFormatDistTri" name="exportFormat" required> <option value="excel">Excel (.xlsx)</option> <option value="csv">CSV (.csv)</option> </select> </div>
                    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function hfc() {
            ...
            if (ts) p.set('tahun', ts.value); else p.delete('tahun'); // Handle jika filter tahun tidak ada
            p.set('page', 1);
            // Gunakan route helper dengan parameter jenisKegiatan
            window.location.href = '{{ route("tim-distribusi.triwulanan.index", ["jenisKegiatan" => $jenisKegiatan]) }}' + '?' + p.toString();
        }
