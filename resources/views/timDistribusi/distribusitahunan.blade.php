...
@push('styles')
    <style>
        ...
        /* [STYLE TAMBAHAN] Tampilkan invalid feedback di modal */
    ...
    {{-- Alert Import Errors --}}
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                 ...
                 <p class="page-subtitle">Kelola data target kegiatan tahunan tim Distribusi</p>
            </div>
        </div>
        ...
        <div class="data-card">
            ...
            <div class="toolbar">
                ...
                <div class="toolbar-right">
                    ...
                    {{-- Dropdown Filter Kegiatan (Cerdas) --}}
                    <div class="filter-group">
            ...
            {{-- Alert Error (dari redirect back) --}}
            @if (session('error'))
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_distribusi }}"></td>
                                {{-- Tampilkan nama dari relasi (bersih) atau lokal (kotor) --}}
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state"> {{-- Colspan 9 --}}
                                    ...
                                    <p class="empty-text">Tidak ada data tahunan ditemukan untuk tahun {{ $selectedTahun }}.</p>
                                     @if($search || $selectedKegiatan) <a href="{{ route('tim-distribusi.tahunan.index', ['tahun' => $selectedTahun]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modern-modal">
                <form action="{{ route('tim-distribusi.tahunan.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info d-flex align-items-start p-3" role="alert">
                            ...
                            <div>
                                <small>
                                    <strong>PENTING:</strong> Pastikan data `nama_kegiatan` di file Excel Anda **sudah valid** dan terdaftar di Master Kegiatan untuk modul **Distribusi Tahunan**.
                                </small>
                            </div>
                        </div>
                        <div class="mb-3">
                            <a href="{{ route('tim-distribusi.tahunan.downloadTemplate') }}" class="btn-action btn-secondary w-100">
            ...
            <form action="{{ route('tim-distribusi.tahunan.store') }}" method="POST" id="tambahForm">
                ...
                <div class="modal-content modern-modal">
                    ...
                    <div class="modal-body">
                        ...
                        {{-- Input Kegiatan dengan Hidden ID --}}
            ...
            <form id="deleteForm" method="POST"> 
                ...
                <div class="modal-content modern-modal">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="delete-text" id="deleteModalBody">Hapus data Tahunan ini?</p> 
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn-secondary" data-bs-dismiss="modal">Batal</button>
...
@push('scripts')
{{-- Ini adalah skrip LENGKAP yang disesuaikan untuk 'tahunan' --}}
<script>
    /**
    ...
    // --- URL Basis BARU ---
    const distribusiTahunanBaseUrl = `{{ route('tim-distribusi.tahunan.index') }}`;
    const currentJenisKegiatan = 'Tahunan'; // Hanya untuk teks
    ...
    function editData(id) {
        ...
        editForm.action = `${distribusiTahunanBaseUrl}/${id}`;
        ...
        fetch(`${distribusiTahunanBaseUrl}/${id}/edit`)
            .then(response => { 
    ...
    function deleteData(id) {
        ...
        deleteForm.action = `${distribusiTahunanBaseUrl}/${id}`;
        ...
        document.getElementById('deleteModalBody').innerText = `Apakah Anda yakin ingin menghapus data ${currentJenisKegiatan} ini?`;
    ...
    function clearFormErrors(form) { 
        ...
        form.querySelectorAll('.invalid-feedback[data-field]').forEach(el => {
            el.textContent = '';
            el.style.display = 'none'; // Sembunyikan juga
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(Route::has('tim-distribusi.tahunan.searchKegiatan'))
            const kegiatanSearchUrl = `{{ route("tim-distribusi.tahunan.searchKegiatan") }}?`; 
        ...
        @else 
            console.warn('Rute tim-distribusi.tahunan.searchKegiatan tidak ditemukan.'); 
        @endif

        @if(Route::has('tim-distribusi.tahunan.searchPetugas'))
            const petugasSearchUrl = '{{ route("tim-distribusi.tahunan.searchPetugas") }}?';
        ...
        @else 
            console.warn('Rute tim-distribusi.tahunan.searchPetugas tidak ditemukan.'); 
        @endif
        ...
        bdb?.addEventListener('click', () => {
            ...
            df.action = '{{ route("tim-distribusi.tahunan.bulkDelete") }}'; 
        ...
        function hfc() {
            // [PERBAIKAN] Gunakan URL basis yang benar
            const cu = new URL('{{ route("tim-distribusi.tahunan.index") }}');
            // Ambil parameter LAMA dari URL browser, BUKAN dari URL route
        ...
        @if (session('error_modal') == 'editDataModal' && $errors->getBag('editForm')->any() && session('edit_id')) 
            ...
            if (eid_fb) { 
                ...
                if(emef) { 
                    ...
                    edf.action = `${distribusiTahunanBaseUrl}/${eid_fb}`; 
                    ...
                    emef.addEventListener('shown.bs.modal', () => {
                         // Tampilkan error dari error bag 'editForm'
