...
{{-- Judul Disesuaikan --}}
@section('title', strtoupper(str_replace('-', ' ', $jenisKegiatan)) . ' - Produksi Bulanan - Sitokcer')
...
@section('content')
    ...
    {{-- Alert Import Errors --}}
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                <h2 class="page-title">List Target Kegiatan Bulanan: {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }}</h2>
        ...
        <div class="data-card">
            ...
            <div class="toolbar">
                ...
                <div class="toolbar-right">
                    ...
                    <form action="{{ route('tim-produksi.bulanan.index', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" class="search-form">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_produksi_bulanan }}"></td>
                                ...
                                <td class="td-action">
                                    <div class="action-buttons">
                                        <button class="btn-icon btn-icon-edit" title="Edit" data-bs-toggle="modal" data-bs-target="#editDataModal" onclick="editData({{ $item->id_produksi_bulanan }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg> </button>
                                        <button class="btn-icon btn-icon-delete" title="Hapus" data-bs-toggle="modal" data-bs-target="#deleteDataModal" onclick="deleteData({{ $item->id_produksi_bulanan }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> </button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    @if($search || $selectedKegiatan) <a href="{{ route('tim-produksi.bulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modern-modal">
                <form action="{{ route('tim-produksi.bulanan.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info d-flex align-items-start p-3" role="alert">
                            ...
                            <div>
                                <small>
                                    <strong>PENTING:</strong> Pastikan data `nama_kegiatan` di file Excel Anda **sudah valid** dan terdaftar di Master Kegiatan untuk modul **Produksi Bulanan**.
                                </small>
                            </div>
                        </div>
                        <div class="mb-3">
                            <a href="{{ route('tim-produksi.bulanan.downloadTemplate') }}" class="btn-action btn-secondary w-100">
    ...
    <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('tim-produksi.bulanan.export', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" id="exportForm">
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data Bulanan {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }}</h5> <p class="modal-subtitle">Pilih opsi ekspor</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
...
{{-- 
====================================================================
== SEMUA 'caturwulan' TELAH DIUBAH MENJADI 'bulanan' ==
...
<script>
    ...
    // --- [PERBAIKAN URL BASIS] ---
    const bulananRootUrl = `{{ url('tim-produksi/bulanan') }}`;
    const bulananPageUrl = `{{ route('tim-produksi.bulanan.index', ['jenisKegiatan' => $jenisKegiatan]) }}`;
    ...
    function editData(id) {
        ...
        editForm.action = `${bulananRootUrl}/${id}`;
    ...
    function deleteData(id) {
        ...
        deleteForm.action = `${bulananRootUrl}/${id}`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(Route::has('tim-produksi.bulanan.searchKegiatan'))
            const kegiatanSearchUrl = `{{ route("tim-produksi.bulanan.searchKegiatan", ['jenisKegiatan' => $jenisKegiatan]) }}?`;
        ...
        @else 
            console.warn('Rute tim-produksi.bulanan.searchKegiatan tidak ditemukan.'); 
        @endif

        @if(Route::has('tim-produksi.bulanan.searchPetugas'))
            const petugasSearchUrl = '{{ route("tim-produksi.bulanan.searchPetugas") }}?';
        ...
        @else 
            console.warn('Rute tim-produksi.bulanan.searchPetugas tidak ditemukan.'); 
        @endif
        ...
        function hfc() {
            const cu = new URL(bulananPageUrl);
        ...
        @if (session('error_modal') == 'editDataModal' && $errors->getBag('editForm')->any() && session('edit_id')) 
            ...
            if (eid_fb) { 
                ...
                if(emef) { 
                    ...
                    edf.action = `${bulananRootUrl}/${eid_fb}`; 
