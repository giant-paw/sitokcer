...
@section('title', strtoupper($jenisKegiatan) . ' - Produksi Caturwulanan - Sitokcer') {{-- Title disesuaikan --}}
...
@push('styles')
    <style>
        ...
        /* Lebar kolom bisa disesuaikan di sini jika perlu */
        .th-checkbox { width: 5%; }
    ...
    <div class="container-fluid px-4 py-4">
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                <h2 class="page-title">List Target Survei {{ ucwords($jenisKegiatan) }} (Caturwulanan)</h2>
                <p class="page-subtitle">Kelola data target survei {{ $jenisKegiatan }} tim Produksi</p>
            </div>
        </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData ?? [] as $item)
                            <tr>
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_produksi_caturwulanan }}"></td> {{-- ID caturwulanan --}}
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    @if($search || $selectedKegiatan) <a href="{{ route('tim-produksi.caturwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun ?? date('Y')]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('tim-produksi.caturwulanan.import') }}" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <a href="{{ route('tim-produksi.caturwulanan.downloadTemplate') }}"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('tim-produksi.caturwulanan.export', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" id="exportForm">
                 <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data caturwulanan {{ strtoupper($jenisKegiatan) }}</h5> <p class="modal-subtitle">Pilih opsi ekspor Anda</p> </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group"> <label for="exportDataRangeDist" class="form-label">Jangkauan Data</label> <select class="form-select" id="exportDataRangeDist" name="dataRange" required> <option value="all">Semua Data ({{ $listData->total() }})</option> <option value="current_page">Halaman Ini ({{ $listData->count() }})</option> </select> </div>
                        ...
                        <div class="form-group"> <label for="exportExportFormatDist" class="form-label">Format File</label> <select class="form-select" id="exportExportFormatDist" name="exportFormat" required> <option value="excel">Excel (.xlsx)</option> <option value="csv">CSV (.csv)</option> </select> </div>
                    </div>
                    <div class="modal-footer">
                           ...
                           <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-input @error('tanggal_pengumpulan') is-invalid @enderror" id="edit_tanggal_pengumpulan" name="tanggal_pengumpulan" value="{{ old('tanggal_pengumpulan') }}">
                                    <div class="invalid-feedback" data-field="tanggal_pengumpulan">@error('tanggal_pengumpulan') {{ $message }} @enderror</div>
                                </div>
                            </div>
                        </div>
...
        const ProduksiCaturwulananBaseUrl = '/tim-produksi/caturwulanan';
...
            editForm.action = `${ProduksiCaturwulananBaseUrl}/${id}`;
            clearFormErrors(editForm);

            // --- PERBAIKAN 2: Gunakan URL baru ---
            fetch(`${ProduksiCaturwulananBaseUrl}/${id}/edit`)
                .then(response => {
                    if (!response.ok) {
...
            // --- PERBAIKAN 2: Gunakan URL baru ---
            deleteForm.action = `${ProduksiCaturwulananBaseUrl}/${id}`;
...
                @else
                console.warn('Rute tim-produksi.caturwulanan.searchPetugas tidak ada.');
            @endif
...
                            editForm.action = `${ProduksiCaturwulananBaseUrl}/${eid}`;
