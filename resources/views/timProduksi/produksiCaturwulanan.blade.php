...
{{-- Judul Disesuaikan --}}
@section('title', strtoupper(str_replace('-', ' ', $jenisKegiatan)) . ' - Produksi Caturwulan - Sitokcer')
@section('header-title', 'List Target Kegiatan: ' . ucwords(str_replace('-', ' ', $jenisKegiatan)))
...
@push('styles')
    <style>
        ...
        .th-checkbox { width: 5%; }
        .data-table th:nth-child(2) { width: 20%; } /* Nama Kegiatan */
        ...
        .th-action { width: 8%; }  /* Aksi */

        /* Style untuk form select */
        ...
        .modern-modal .is-invalid ~ .invalid-feedback {
            display: block; 
        }
    </style>
@endpush
...
@section('content')
    ...
    {{-- Alert Import Errors --}}
        ...
        <div class="page-header mb-4">
            <div class="header-content">
                <h2 class="page-title">List Target Kegiatan Caturwulan: {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }}</h2>
                <p class="page-subtitle">Kelola data target kegiatan {{ $jenisKegiatan }} tim Produksi</p>
            </div>
        </div>
        ...
        <div class="data-card">
            ...
            <div class="toolbar">
                ...
                <div class="toolbar-right">
                    ...
                    {{-- [LOGIKA BARU] Dropdown Filter Kegiatan (Cerdas) --}}
                    <div class="filter-group">
                        ...
                        <select class="filter-select" id="kegiatanSelect" name="kegiatan">
                            ...
                            @foreach ($kegiatanCounts ?? [] as $kegiatan)
                                {{-- Gunakan filter_value dan display_name dari query COALESCE --}}
                    ...
                    {{-- Search form [PERBAIKAN ROUTE] --}}
                    <form action="{{ route('tim-produksi.caturwulanan.index', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" class="search-form">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($listData as $item)
                            <tr>
                                {{-- [PERBAIKAN ID] --}}
                                <td class="td-checkbox"><input type="checkbox" class="table-checkbox row-checkbox" value="{{ $item->id_produksi_caturwulanan }}"></td>
                                ...
                                <td class="td-action">
                                    <div class="action-buttons">
                                        {{-- [PERBAIKAN ID] --}}
                                        <button class="btn-icon btn-icon-edit" title="Edit" data-bs-toggle="modal" data-bs-target="#editDataModal" onclick="editData({{ $item->id_produksi_caturwulanan }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg> </button>
                                        {{-- [PERBAIKAN ID] --}}
                                        <button class="btn-icon btn-icon-delete" title="Hapus" data-bs-toggle="modal" data-bs-target="#deleteDataModal" onclick="deleteData({{ $item->id_produksi_caturwulanan }})"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> </button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="empty-state">
                                    ...
                                    <p class="empty-text">Tidak ada data {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }} ditemukan untuk tahun {{ $selectedTahun }}.</p>
                                    {{-- [PERBAIKAN ROUTE] --}}
                                    @if($search || $selectedKegiatan) <a href="{{ route('tim-produksi.caturwulanan.index', ['jenisKegiatan' => $jenisKegiatan, 'tahun' => $selectedTahun]) }}" class="empty-link">Reset filter</a> @endif
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modern-modal">
                <form action="{{ route('tim-produksi.caturwulanan.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info d-flex align-items-start p-3" role="alert">
                            ...
                            <div>
                                <small>
                                    <strong>PENTING:</strong> Pastikan data `nama_kegiatan` di file Excel Anda **sudah valid** dan terdaftar di Master Kegiatan untuk modul **Produksi Caturwulanan**.
                                </small>
                            </div>
                        </div>
                        <div class="mb-3">
                            {{-- [PERBAIKAN ROUTE] --}}
                            <a href="{{ route('tim-produksi.caturwulanan.downloadTemplate') }}" class="btn-action btn-secondary w-100">
    ...
    {{-- Modal Export [PERBAIKAN ROUTE] --}}
    <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('tim-produksi.caturwulanan.export', ['jenisKegiatan' => $jenisKegiatan]) }}" method="GET" id="exportForm">
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Ekspor Data Caturwulan {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }}</h5> <p class="modal-subtitle">Pilih opsi ekspor</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
                        <div class="form-group"> <label for="exportDataRange" class="form-label">Jangkauan Data</label> <select class="form-select" id="exportDataRange" name="dataRange" required> <option value="all">Semua Data</option> <option value="current_page">Halaman Ini</option> </select> </div>
                        <div class="form-group"> <label for="exportDataFormat" class="form-label">Format Nilai Tanggal</label> <select class="form-select" id="exportDataFormat" name="dataFormat"> <option value="formatted_values" selected>Format Tampilan (dd/mm/yyyy)</option> <option value="raw_values">Nilai Asli Database (yyyy-mm-dd)</option> </select> <small class="form-text text-muted">Pilih "Raw Values" untuk olah data.</small> </div>
                        <div class="form-group"> <label for="exportExportFormat" class="form-label">Format File</label> <select class="form-select" id="exportExportFormat" name="exportFormat" required> <option value="excel">Excel (.xlsx)</option> <option value="csv">CSV (.csv)</option> </select> </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn-secondary" data-bs-dismiss="modal">Batal</button>
    ...
    {{-- Modal Tambah Data [PERBAIKAN ROUTE] --}}
    <div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-lg modal-dialog-centered"> 
            <form action="{{ route('tim-produksi.caturwulanan.store') }}" method="POST" id="tambahForm">
                @csrf
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Tambah Data {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }} Baru</h5> <p class="modal-subtitle">Isi form di bawah</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="form-group autocomplete-container">
                            ...
                            <input type="text" class="form-input @error('nama_kegiatan') is-invalid @enderror"
                                ...
                                placeholder="Ketik {{ strtoupper(str_replace('-', ' ', $jenisKegiatan)) }}..." required autocomplete="off">
                            ...
                            <div class="invalid-feedback" data-field="nama_kegiatan" style="display: none;">@error('nama_kegiatan') {{ $message }} @enderror</div>
                            <div class="invalid-feedback" data-field="master_kegiatan_id" style="display: none;">@error('master_kegiatan_id') {{ $message }} @enderror</div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="BS_Responden" style="display: none;">@error('BS_Responden') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group autocomplete-container">
                                    ...
                                    <div class="invalid-feedback" data-field="pencacah" style="display: none;">@error('pencacah') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group autocomplete-container">
                                    ...
                                    <div class="invalid-feedback" data-field="pengawas" style="display: none;">@error('pengawas') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="target_penyelesaian" style="display: none;">@error('target_penyelesaian') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="flag_progress" style="display: none;">@error('flag_progress') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="tanggal_pengumpulan" style="display: none;">@error('tanggal_pengumpulan') {{ $message }} @enderror</div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="editDataModal" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-lg modal-dialog-centered"> 
            <form id="editForm" method="POST"> 
                ...
                <div class="modal-content modern-modal">
                    <div class="modal-header">
                        <div class="modal-header-content"> <h5 class="modal-title">Edit Data Caturwulan</h5> <p class="modal-subtitle">Perbarui informasi data</p> </div>
                        <button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg> </button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="form-group autocomplete-container">
                            ...
                            <div class="invalid-feedback" data-field="nama_kegiatan" style="display: none;">@error('nama_kegiatan', 'editForm') {{ $message }} @enderror</div>
                            <div class="invalid-feedback" data-field="master_kegiatan_id" style="display: none;">@error('master_kegiatan_id', 'editForm') {{ $message }} @enderror</div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="BS_Responden" style="display: none;">@error('BS_Responden', 'editForm') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group autocomplete-container">
                                    ...
                                    <div class="invalid-feedback" data-field="pencacah" style="display: none;">@error('pencacah', 'editForm') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group autocomplete-container">
                                    ...
                                    <div class="invalid-feedback" data-field="pengawas" style="display: none;">@error('pengawas', 'editForm') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="target_penyelesaian" style="display: none;">@error('target_penyelesaian', 'editForm') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="flag_progress" style="display: none;">@error('flag_progress', 'editForm') {{ $message }} @enderror</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="invalid-feedback" data-field="tanggal_pengumpulan" style="display: none;">@error('tanggal_pengumpulan', 'editForm') {{ $message }} @enderror</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <form id="deleteForm" method="POST"> 
                ...
                <div class="modal-content modern-modal">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="delete-text" id="deleteModalBody">Hapus data {{ ucwords(str_replace('-', ' ', $jenisKegiatan)) }} ini?</p> 
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn-secondary" data-bs-dismiss="modal">Batal</button>
...
{{-- 
====================================================================
== [PERBAIKAN TOTAL PADA JAVASCRIPT] ==
== TYPO 'caturwulan' vs 'caturwulanan' TELAH DIPERBAIKI SEMUA ==
== URL UNTUK EDIT/DELETE TELAH DIPERBAIKI AGAR TIDAK KONFLIK ==
== PERBAIKAN KLIK SUBMENU TELAH DITAMBAHKAN ==
====================================================================
--}}
<script>
    /**
        ...
        input.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (activeSuggestionIndex > -1) {
                    ...
                    const idContent = selectedSuggestion.getAttribute('data-id'); 
    ...
    // --- [PERBAIKAN URL BASIS v2] ---
    // URL ini adalah 'akar' dari modul, misal: .../tim-produksi/caturwulanan
    // Digunakan untuk edit, delete, update
    const caturwulananRootUrl = `{{ url('tim-produksi/caturwulanan') }}`;
    
    // URL ini adalah halaman SPESIFIK saat ini, misal: .../caturwulanan/ubinan
    // Digunakan untuk filter dan search
    const caturwulananPageUrl = `{{ route('tim-produksi.caturwulanan.index', ['jenisKegiatan' => $jenisKegiatan]) }}`;
    
    const currentJenisKegiatan = '{{ $jenisKegiatan }}'; 

    /**
     * [PERBAIKAN] Mengisi data ke Modal Edit
     */
    function editData(id) {
        ...
        // [PERBAIKAN] Gunakan caturwulananRootUrl agar URL-nya benar
        // Hasil: .../tim-produksi/caturwulanan/123
        editForm.action = `${caturwulananRootUrl}/${id}`;
        ...
        // [PERBAIKAN] Gunakan caturwulananRootUrl agar URL-nya benar
        // Hasil: .../tim-produksi/caturwulanan/123/edit
        fetch(`${caturwulananRootUrl}/${id}/edit`)
            .then(response => { 
                if (!response.ok) { return response.text().then(text => { throw new Error(text || 'Data tidak ditemukan'); }); } 
                return response.json(); 
            })
            .then(data => {
    ...
    /**
     * [PERBAIKAN] Mengatur form action untuk HAPUS SATU data
     */
    function deleteData(id) {
        ...
        // [PERBAIKAN] Gunakan caturwulananRootUrl agar URL-nya benar
        // Hasil: .../tim-produksi/caturwulanan/123
        deleteForm.action = `${caturwulananRootUrl}/${id}`;
    ...
    function clearFormErrors(form) { 
        ...
        form.querySelectorAll('.invalid-feedback[data-field]').forEach(el => {
            el.textContent = '';
    ...
    function showFormErrors(form, errors) { 
        for (const [field, messages] of Object.entries(errors)) { 
            ...
            if (field === 'master_kegiatan_id') {
                 ...
                 if(visibleErrorDiv) {
                    visibleErrorDiv.textContent = messages[0];
                    visibleErrorDiv.style.display = 'block'; 
                 }
            }
        } 
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- Init Autocomplete (Gunakan route baru) [PERBAIKAN ROUTE] ---
        @if(Route::has('tim-produksi.caturwulanan.searchKegiatan'))
            // [PERBAIKAN] Gunakan caturwulananPageUrl
            const kegiatanSearchUrl = `{{ route("tim-produksi.caturwulanan.searchKegiatan", ['jenisKegiatan' => $jenisKegiatan]) }}?`;
        ...
        @else 
            console.warn('Rute tim-produksi.caturwulanan.searchKegiatan tidak ditemukan.'); 
        @endif

        @if(Route::has('tim-produksi.caturwulanan.searchPetugas'))
            const petugasSearchUrl = '{{ route("tim-produksi.caturwulanan.searchPetugas") }}?';
        ...
        @else 
            console.warn('Rute tim-produksi.caturwulanan.searchPetugas tidak ditemukan.'); 
        @endif
        ...
        // --- Select All & Bulk Delete [PERBAIKAN ROUTE & MODAL SHOW] ---
        ...
        bdb?.addEventListener('click', () => {
            ...
            df.action = '{{ route("tim-produksi.caturwulanan.bulkDelete") }}'; 
            ...
            bootstrap.Modal.getOrCreateInstance(dme).show();
        });

        // --- Filters Per Page & Tahun & Kegiatan [PERBAIKAN ROUTE] ---
        ...
        function hfc() {
            // [PERBAIKAN] Gunakan caturwulananPageUrl
            const cu = new URL(caturwulananPageUrl);
            ...
            if (this && this.id === 'searchInput') {
                 p.set('focus_search', '1');
            }

            window.location.href = cu.pathname + '?' + p.toString();
        }

        if (pps) pps.addEventListener('change', hfc.bind(null));
        if (ts) ts.addEventListener('change', hfc.bind(null));
        if (ks) ks.addEventListener('change', hfc.bind(null));
        ...
        // --- Prevent Form Submit on Enter ---
         if(searchForm) {
            searchForm.addEventListener('submit', (e) => {
                 e.preventDefault(); 
                 hfc.call(searchInput || null); 
            });
         }
        
        // --- Fokus kembali ke search input ---
        ...
        // --- Fallback Error Modals [PERBAIKAN URL] ---
        ...
        @if (session('error_modal') == 'editDataModal' && $errors->getBag('editForm')->any() && session('edit_id')) 
            ...
            if (eid_fb) { 
                ...
                if(emef) { 
                    ...
                    // [PERBAIKAN] Gunakan caturwulananRootUrl
                    edf.action = `${caturwulananRootUrl}/${eid_fb}`; 
                    
                    editData(eid_fb); // Memuat data & .show()
                    
                    const editModalInstance = bootstrap.Modal.getOrCreateInstance(emef);
        ...
        // --- Auto-hide Alerts ---
        const alertList = document.querySelectorAll('.alert-dismissible[role="alert"]:not(.alert-warning)'); 
        ...
        document.querySelectorAll('.submenu a').forEach(link => {
            link.addEventListener('click', function(event) {
                // Hentikan event agar tidak 'naik' ke parent dropdown toggle
